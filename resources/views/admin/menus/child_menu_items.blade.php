					...
					<div class="row">
						@if($value->type == 'Link')
							<div class="col-md-12 form-group">
								<label for="MenuItem{{ $value->id }}Link">{{ __('URL') }}</label>
						...
						<div class="col-md-6 form-group">
							<label for="MenuItem{{ $value->id }}Title">{{ __('Navigation Label') }}</label>
						...
						<div class="col-md-6 form-group XTitleAttribute {{ $screenOption['TitleAttribute']['visibility'] ? '' : 'd-none' }}">
							<label for="MenuItem{{ $value->id }}Attribute">{{ __('Title Attribute') }}</label>
						...
						<div class="col-md-12 form-group XLinkTarget {{ $screenOption['LinkTarget']['visibility'] ? '' : 'd-none' }}">
							<div class="form-check">
								...
								<label class="form-check-label" for="MenuItem{{ $value->id }}Target">{{ __('Open link in a new tab') }}</label>
							</div>
						</div>
						<div class="col-md-12 form-group XCssClasses {{ $screenOption['CssClasses']['visibility'] ? '' : 'd-none' }}">
							<label for="MenuItem[{{ $value->id }}]CssClasses">{{ __('CSS Classes') }}</label>
						...
						<div class="col-md-12 form-group XDescription {{ $screenOption['Description']['visibility'] ? '' : 'd-none' }}">
							<label for="MenuItem[{{ $value->id }}]Description">{{ __('Description') }}</label>
						...
						<div class="col-md-12">
							<a href="javascript:void(0);" class="RemoveItem text-primary" rel="{{ $value->id }}" item-name="{{ $value->title }}">{{ __('Remove') }}</a>
							<span class="text-black mx-2">|</span>
							<a href="javascript:void(0);" rel="{{ $value->id }}" class="CancelItem text-primary"> {{ __('Cancel') }}</a>
						</div>
					</div>
				</div>
			</div>
