    ...
    public function importData($xml, $user_id) {
        ...
        if (isset($xml->blogs->blogs)) {
            ...
            foreach($xml->blogs->blogs as $blogXml) {
                ...
                if (!Blog::whereSlug($blogXml->slug)->exists()) {
                    ...
                    $returnMessage[] = '"'.$blogXml->title.'" Post is Already exist.'; 
                }
            }
        }
        ...
        if (isset($xml->pages->pages)) {
            ...
            function checkAndInsertPage(&$pageXml, $parent_id=Null, $user_id) {

                if (!Page::whereSlug($pageXml->slug)->exists()) {
                    ...
                    if ($pageXml->children->children) {
                        checkAndInsertPage($pageXml->children->children, $pageNewId, $user_id);
                    }
                    ...
                    $returnMessage[] = '"'.$pageXml->title.'" Page is Already exist.'; 
                }
            }

            foreach($xml->pages->pages as $pageXml) {

                checkAndInsertPage($pageXml, (int)$pageXml->parent_id, $user_id);
            }
        }
        ...
        if (isset($xml->menus->menus)) {
            foreach($xml->menus->menus as $menuXml) {
                ...
                if (!Menu::whereSlug($menuXml->slug)->exists()) {
                    ...
                    {
                        if( $menusLocations[$location]['menu'] == (int)$menuXml->id )
                        {
                    ...
                    $returnMessage[] = '"'.$menuXml->title.'" Menu is Already exist.'; 
                }
            }
        }
        /*================ Get data from saved xml file and Save to database ================*/
        ...
        $message = implode(' and ', $returnMessage);
